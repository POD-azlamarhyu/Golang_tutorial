...
func goroutineExample(a []int, c chan int) {
	sum := 0
...
func blockingExample(a []int, c chan int) {
...
func main() {
	...
	go goroutineExample(a,c)
	go blockingExample(a, c)

	// ゴルーチンが終了するのを待つ
	// ゴルーチンから受信した値は、キューのような形で順番に処理される
	sum := <-c
	fmt.Printf("Sum of array: %d\n", sum)
	times := <-c
	fmt.Printf("Product of array: %d\n", times)
